...
def nep_regression_3D(x, y, z):
    ...
    for i in range(X_grid.shape[0]):
        for j in range(X_grid.shape[1]):
            ...
            Z_grid[i, j] = nadaraya_watson(X, z, x_query, h=0.5)  # Вычисление значения на поверхнос

    # Построение графика в 3D
    fig = go.Figure()
    fig.add_trace(go.Scatter3d(x=x, y=y, z=z, mode='markers', marker=dict(color='blue'), name='Исходные данные'))
    fig.add_trace(
