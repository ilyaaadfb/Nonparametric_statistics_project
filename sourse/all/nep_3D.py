...
def nep_regression_3D(x, y, z):
    ...
    for i in range(X_grid.shape[0]):
        for j in range(X_grid.shape[1]):
            ...
            Z_grid[i, j] = nadaraya_watson(X, z, x_query, h=0.1)  # Вычисление значения на поверхности

    # Построение графика в 3D
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.scatter(x, y, z, c='blue', label='Исходные данные')
    ax.plot_surface(X_grid, Y_grid, Z_grid, cmap='gnuplot2_r', alpha=0.5, label='Регрессия Надарая-Ватсон')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.legend()
